...
const CardId = () => {
  ...
  const GetDetails = async () => {
    const strimage = await fetch(
      `https://api.themoviedb.org/3/movie/${id}?api_key=1665407252326530351ed5a76c30158f&language=en-US`
    );
  ...
  const getsimilarMovies = async () => {
    const { data } = await axios.get(
      `https://api.themoviedb.org/3/movie/${id}/similar?api_key=1665407252326530351ed5a76c30158f&language=en-US&page=1`
    );
        ...
        <div className={`${css.content}`}>
          <div className={`${css.one}`}>
            <div>
              <h1>{image.title}</h1>
              <h4>
                {image.runtime} min | {image.release_date} |{" "}
                {image.original_language}{" "}
              </h4>
            </div>
            <h5>
              {image.vote_average} <HeartFilled />
            </h5>
          </div>
          <div className={`${css.two}`}>
            <p>
              {image.overview} <br /> <br /> "{image.tagline}"{" "}
            </p>
            <table className={`${css.table}`}>
              <tr>
                <td
                  className={`${css.td}`}
                  style={{ width: "30%", color: "#B0A4A4" }}
                >
                  Genres:
                </td>
                <td className={`${css.td}`} style={{ width: "70%" }}>
                  {image.genres
                    ? image.genres.map((genre) => genre.name + ", ")
                    : ""}
                </td>
              </tr>
              <tr>
                ...
                <td className={`${css.td}`} style={{ width: "70%" }}>
                  {image.spoken_languages
                    ? image.spoken_languages.map((lang) => lang.name + ", ")
                    : ""}
                </td>
              </tr>
              <tr>
                ...
                <td className={`${css.td}`} style={{ width: "70%" }}>
                  {image.production_companies
                    ? image.production_companies.map((com) => com.name + ", ")
                    : ""}
                </td>
              </tr>
            </table>
